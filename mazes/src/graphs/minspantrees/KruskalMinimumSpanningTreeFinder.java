...
public class KruskalMinimumSpanningTreeFinder<G extends KruskalGraph<V, E>, V, E extends BaseEdge<V, E>>
    ...
    public MinimumSpanningTree<V, E> findMinimumSpanningTree(G graph) {
        ...
        for (V vertex : graph.allVertices()) { // add all vertex as single-element
            msts.makeSet(vertex);
        }
        ...
        for (E edge : edges) {
            int uMSTID = msts.findSet(edge.from());
            int vMSTID = msts.findSet(edge.to());
            if (uMSTID != vMSTID) {
                finalMST.add(edge);
        ...
        if (graph.allEdges().isEmpty() && (graph.allVertices().isEmpty()) || graph.allVertices().size() == 1) {
            return new MinimumSpanningTree.Success<>();
        } else if (graph.allVertices().size() -1 != finalMST.size() || graph.allEdges().isEmpty()) {
